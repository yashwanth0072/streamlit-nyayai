...
def apply_custom_css():
    ...
    css = (
        ...
        "/* Feature cards */\n"
        ".feature-card { position: relative; background: rgba(255, 255, 255, 0.1); padding: 1.5rem; border-radius: 12px; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.2); margin: 1rem 0; border-left: 4px solid " + ACCENT_COLOR + "; transition: all 0.3s ease; cursor: pointer; backdrop-filter: blur(10px); color: " + TEXT_COLOR + "; }\n"
...
def add_sidebar_toggle():
    ...
    with col1:
        if st.button('☰', key='sidebar_toggle', help="Toggle sidebar visibility"):
...
def render_feature_card_with_navigation(title: str, description: str, icon: str, page_name: str):
    """Render a feature card that navigates in the same tab when clicked.

    Implementation: wrap the card in an anchor with `?page=` query param.
    The app reads this param in main() and updates session state, then clears it.
    """
    import urllib.parse

    # Encode page name for URL
    encoded_page = urllib.parse.quote_plus(page_name)

    # Render the card HTML wrapped in an anchor for same-tab navigation
    card_html = f"""
    <a href="?page={encoded_page}" target="_self" style="text-decoration: none; color: inherit;">
        <div class="feature-card" style="position: relative; cursor: pointer; margin-bottom: 0.5rem;">
            <h3>{icon} {title}</h3>
            <p>{description}</p>
...
def render_ipc_flashcard(section_data: dict, card_id: str):
    ...
    with container:
        ...
        with col2:
            if st.button(f"🔄 Flip Card", key=f"flip_btn_{card_id}", use_container_width=True):
                ...
                <div class="flashcard {flip_class}">
                    <div class="flashcard-front">
                        <div class="flashcard-content">
                            <div class="flashcard-section-number">§ {section_data['section']}</div>
            ...
            <div class="flashcard-container" id="card_{card_id}">
                <div class="flashcard {flip_class}">
                    <div class="flashcard-back">
                        <div class="flashcard-content">
                            <div class="flashcard-section-number">⚖️ Punishment</div>
...
def render_status_indicator(is_online: bool):
    ...
    if is_online:
        st.markdown('<p class="status-online">🟢 AI Online - Full assistance available</p>', unsafe_allow_html=True)
    else:
        st.markdown('<p class="status-offline">🟡 Offline Mode - Basic assistance available. Configure API key for full features.</p>', unsafe_allow_html=True)
